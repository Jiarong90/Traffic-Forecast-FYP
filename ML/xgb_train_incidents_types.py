...
# Pulling incident_type instead of has_incident
train_df = con.execute(f"""
    SELECT 
        sb, sb_tm5, sb_tm10, sb_tm15, road_category, 
        mid_lat, mid_lon, lat_bin, lon_bin, incident_type,
        y_tp15, snapshot_ts
    FROM read_parquet('{FEAT}')
...
# Fill NaNs for the text type
train_df["incident_type"] = train_df["incident_type"].fillna("None")
test_df["incident_type"] = test_df["incident_type"].fillna("None")

# Create the binary has_incident flag directly in Pandas
train_df["has_incident"] = (train_df["incident_type"] != "None").astype(int)
test_df["has_incident"] = (test_df["incident_type"] != "None").astype(int)

all_types = list(set(train_df["incident_type"].unique()) | set(test_df["incident_type"].unique()))
type_map = {t: i for i, t in enumerate(all_types)}

train_df["inc_type_code"] = train_df["incident_type"].map(type_map)
test_df["inc_type_code"] = test_df["incident_type"].map(type_map)

features = [
    "sb", "sb_tm5", "sb_tm10", "sb_tm15", 
    "road_category", "mid_lat", "mid_lon", 
    "has_incident", "inc_type_code"
]
...
print("Training XGBoost with Incident Types...")
