...
# MASTER PARQUET FILE
# included all from t-30 to t+30 in one go
# also performed join on incidents and road
...
DAYS = 21
OUT = os.path.join(OUT_DIR, f"features_{DAYS}d_inc_v3.parquet")
...
COPY (
    WITH traffic AS (
        SELECT
            ...
            LAG(speed_band, 4) OVER w AS sb_tm20,
            LAG(speed_band, 5) OVER w AS sb_tm25,
            LAG(speed_band, 6) OVER w AS sb_tm30,
            ...
            LEAD(speed_band, 4) OVER w AS y_tp20,
            LEAD(speed_band, 5) OVER w AS y_tp25,
            LEAD(speed_band, 6) OVER w AS y_tp30
    ...
    SELECT
        link_id,
        snapshot_ts,
        sb, sb_tm5, sb_tm10, sb_tm15, sb_tm20, sb_tm25, sb_tm30,
        hour_sg, dow_sg,
        y_tp5, y_tp10, y_tp15, y_tp20, y_tp25, y_tp30,
        road_category,
